...
The following table lists the dialog variables that are both read from and written to by several of the library
access services. The variables are in the function pool, unless (SHARED) is in the description. For more
details about function pool variables written by other services, see z/OS ISPF Services Guide.
...
Dialog variables
© Copyright IBM Corp. 1980, 2025 349
...
ZEDTMCMD Char 8 Any Edit macro (SHARED) The edit command entered that caused
an edit macro to run. Can be the macro name or
other name is the edit DEFINE command was used
to define an alias.
...
recovery.
Dialog variables
350  z/OS: z/OS ISPF Dialog Developer's Guide and Reference
...
Variable
Name Format Length Service (Access) Description
ZEDUSER Char 2 EDIT (R)
EDREC(W)
(SHARED) User data table extension for standard edit
recovery.
...
ZEIUSER Char 2 EDIF (R)
EDIREC(W)
(SHARED) User data table extension variable for EDIF
edit recovery.
...
Dialog variables
Appendix D. Dialog variables  351
...
Guide.
Dialog variables
352  z/OS: z/OS ISPF Dialog Developer's Guide and Reference
...
Variable
Name Format Length Service (Access) Description
ZLMACENT Char 6 Any Edit line
command macro
(SHARED) The edit line command that was entered
to run the macro, excluding any suffixes that were
entered to indicate a block command.
...
RMODE of the member.
Dialog variables
354  z/OS: z/OS ISPF Dialog Developer's Guide and Reference
...
Dialog variables
356  z/OS: z/OS ISPF Dialog Developer's Guide and Reference
...
are 19xx.
Dialog variables
Appendix D. Dialog variables  357

## Page 386

Dialog variables
358  z/OS: z/OS ISPF Dialog Developer's Guide and Reference
